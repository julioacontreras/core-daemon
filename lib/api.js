...
class RPC {
  ...
  save(writePath, callback) {
    ...
    this.shares.forEach((val, nodeId) => {
      const filename = utils.getFilename(val.path)
      snapshot.push({
        path: val.path,
        id: utils.createHash(filename)
      });
    });
   ...
   */
   ...
   /**
   * Cleans the node with the given node ID
   * @param {String} nodeId
   * @param {RPC~destroyCallback}
   */
  clean(nodeId, callback) {
    this._log(`received clean command for ${nodeId}`);
